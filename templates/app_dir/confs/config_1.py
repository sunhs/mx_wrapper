...
OPTIMIZER = "adam"
OPT_PARAMS = {"learning_rate": 1e-3, "wd": 5e-4}
PARAM_GROUPS = [
    {"params": ".*", "lr_mult": 0.1, "wd_mult": 0.1},
    {"params": [".*dense"], "lr_mult": 1, "wd_mult": 1},
]
GPUS = []
NUM_WORKERS = 8
...
LOG_PKL = os.path.join(MODEL_DIR, "log.pkl")
if not os.path.exists(LOG_PKL):
    with open(LOG_PKL, "wb") as f:
        log = {"train": {"loss": []}, "test": {"loss": []}}
        pickle.dump(log, f)

TRAIN_LOG_PKL = os.path.join(MODEL_DIR, "train_log.pkl")
if not os.path.exists(TRAIN_LOG_PKL):
